...
//! Structures for PointerEvent and MouseEvent

use crate::kurbo::{Point, Vec2};
use crate::{Cursor, Data, Modifiers, MouseButton, MouseButtons, PointerId, PointerType};

/// Data carried by a generic pointer event
///
/// A Pointer is an abstract concept that can represent either
/// a touch event, a stylus event or a mouse event.
#[derive(Debug, Clone, PartialEq)]
pub struct PointerEvent {
    /// Identier common to all pointer events generated by the same source
    /// For example, two fingers moving on a touch screen have two distinct
    /// ids that can be used to differentiate the respective events.
    /// There is no garantuee that an id remains the same when the event
    ...
    /// Normalized pressure exerted on a touch or stylus generated event
    /// pressure value is between `0.0` and `1.0`
    pub pressure: f64,
}
